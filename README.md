...
- выгрузка адресов для внешних скриптов (`usr.ccd2env [файл]`): обход
  папок-профилей (включая отключенные, у которых CCD лежит как
  `<ccd>.disabled`) и bash-файл с переменными `<CN><ccd_suffix>=<ip>` —
  имя переменной повторяет имя CCD-файла (`ccd.2fa` → `<CN>_2fa`);
- подстановка этих переменных в правила фаервола
  (`ip2var <файл переменных> <скрипт> [результат]`): в скрипте с командами
  `iptables` литеральные адреса заменяются на `${переменная}`, исходный
  файл не меняется. Вызов `checkVarsNotEmpty` из `_lib` в начале такого
  скрипта (после `. ccd.env`) проверяет, что все используемые в нем
  переменные имеют значение — иначе правило уйдет в ядро битым;
...
# имя клиентского конфига на инстанс (по умолчанию <prefix>_<CN>_<instance>.ovpn):
...
Так же устроен суффикс клиентского конфига в папке пользователя — атрибут
`<instance>_conf_suffix`: пустая строка — конфиг без суффикса
(`<prefix>_<CN>.ovpn`), иначе `<prefix>_<CN><суффикс>.ovpn` (и
`<prefix>_<CN><суффикс>_connect.ovpn`). Если атрибут не задан, суффикс =
`_<instance>`.
